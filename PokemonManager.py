...
class PokemonManager:
    ...
    def get_relationship_g(self,type1, type2): # scatterplot of attack and defense
        ...
        ax = sns.scatterplot(data=self.__pokemon_data, x=type1, y=type2, hue="Generation")
        figure.suptitle("Scatterplot of attack and defense")
        return figure
    
    def get_attribute_dis_g(self, attribute): # distribution graph of attack
        figure = plt.figure(figsize=(12, 6))
        ax = sns.histplot(data=self.__pokemon_data[attribute],)
        return figure
        
    def get_generation_part_to_whole_g(self): # part of whole graph of generation
        # figure = plt.figure(figsize=(12, 6))
        plot = self.__pokemon_data["Generation"].value_counts().sort_index().plot(kind="pie", y="Generation", title="lol", autopct="%.2f%%", figsize=(12,6), legend=True)
        return plot
